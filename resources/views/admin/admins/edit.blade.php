...
@section('page_title' ,trans('pages/admin/admins.edit_page_title'))
@section('breadcrumb')
    <x-group.bread-crumb
        page-tittle="{{trans('pages/admin/admins.edit_page_title')}}"
        :indexes="[
        ['name'=> ucwords(trans('pages/admin/admins.index_page_title')) , 'route'=> route('admin.admins.index')],
        ['name'=> ucwords(trans('pages/admin/admins.edit_page_title')) , 'current' => true ],
    ]"
    />
@endsection
@section('content')
    ...
    <x-form.form
        method="post"
        action="{{ route('admin.profile.saveDetails' , $model[$model::PK]) }}"
    >

        <x-form.card col="col-12 row" title="{{trans('app.edit').trans('app.edit')}}">
